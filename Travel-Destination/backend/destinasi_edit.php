...
$kode_destinasi = $_GET["ubahdestinasi"];
$edit = mysqli_query($conn, "SELECT * from destinasiwisata WHERE kode_destinasi = '$kode_destinasi'");
...
if (isset($_POST['ubah'])) {
    ...
    $namafoto = $_FILES['fotodestinasi']['name']; /** untuk menampung data foto atau gambar **/
    $file_tmp = $_FILES["fotodestinasi"]["tmp_name"];
    $file_size = $_FILES["fotodestinasi"]["size"];

    //mysqli_query($conn, "INSERT INTO destinasiwisata VALUES ('$kode_destinasi', '$nama_destinasi', '$alamat_destinasi', '$keterangan_destinasi', '$kode_kecamatan', '$namafoto')");
    if ($file_size > 2097152) {
        echo "<script>alert('Error: Ukuran file terlalu besar! Maksimal 2MB.');</script>";
    } else {
        if ($namafoto == "") {
            mysqli_query($conn, "UPDATE destinasiwisata SET nama_destinasi = '$nama_destinasi', alamat_destinasi = '$alamat_destinasi', keterangan_destinasi = '$keterangan_destinasi', kategori_ID = '$kategori_ID', kode_kabupaten = '$kode_kabupaten' WHERE kode_destinasi = '$kode_destinasi'");
        } else {
            mysqli_query($conn, "UPDATE destinasiwisata SET nama_destinasi = '$nama_destinasi', alamat_destinasi = '$alamat_destinasi', keterangan_destinasi = '$keterangan_destinasi', kategori_ID = '$kategori_ID', kode_kabupaten = '$kode_kabupaten', foto_destinasi = '$namafoto' WHERE kode_destinasi = '$kode_destinasi'");
        }        
...
<form method="POST" enctype="multipart/form-data">
    <div class="row mb-3 mt-5">
        ...
        <div class="col-sm-10">
            <input type="text" class="form-control" id="kodeDes" name="inputKODE" value="<?php echo $row_edit["kode_destinasi"]?>"readonly > 
        </div>
    </div>
    <div class="row mb-3">
        <label for="namaDes" class="col-sm-2 col-form-label">Nama Destinasi Wisata</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="namaDes" name="inputNAMA" value="<?php echo $row_edit["nama_destinasi"]?>">
        </div>
    </div>
    <div class="row mb-3">
        <label for="alamatDes" class="col-sm-2 col-form-label">Alamat Destinasi Wisata</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="alamatDes" name="inputALAMAT" value="<?php echo $row_edit["alamat_destinasi"]?>">
        </div>
    </div>
    <div class="row mb-3">
        <label for="ketDes" class="col-sm-2 col-form-label">Keterangan Destinasi Wisata</label>
        <div class="col-sm-10">
            <input type="text" class="form-control" id="ketDes" name="inputKET" value="<?php echo $row_edit["keterangan_destinasi"]?>">
        </div>
    </div>
    ...
    <div class="row mb-3">
        ...
        <div class="col-sm-10">
            ...
            <option><?php echo $row_edit["kode_kabupaten"]?></option>
                <?php if(mysqli_num_rows($datakabupaten) > 0){?>
